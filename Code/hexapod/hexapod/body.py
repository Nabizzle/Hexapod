...
def bodyPos(pitch = 0, roll = 0, yaw = 0, Tx = 0, Ty = 0, Tz = 0, body_offset = 85):
    ...
    body_0 = np.matmul(inv(body_rot), np.array([[body_offset / 2, body_offset * sin(pi / 3), 0]]).T)
    body_1 = np.matmul(inv(body_rot), np.array([[body_offset, 0, 0]]).T)
    body_2 = np.matmul(inv(body_rot), np.array([[body_offset / 2, -body_offset * sin(pi / 3), 0]]).T)
    body_3 = np.matmul(inv(body_rot), np.array([[-body_offset / 2, -body_offset * sin(pi / 3), 0]]).T)
    body_4 = np.matmul(inv(body_rot), np.array([[-body_offset, 0, 0]]).T)
    body_5 = np.matmul(inv(body_rot), np.array([[-body_offset / 2, body_offset * sin(pi / 3), 0]]).T)
